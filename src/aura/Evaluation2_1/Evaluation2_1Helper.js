    ...
    // Null , Undefined , '' 체크
        ...
        doSave: function (component, event, helper, draftValues) {
            ...
            action.setCallback(this, function (response) {
                var state = response.getState();
                if (state === "SUCCESS") {
                    helper.showToast('success', $A.get("$Label.c.SuccessfullySaved"));
        ...
        deleteRow: function (component, event, helper, listDeleteTargetId) {
            ...
            action.setCallback(this, function (response) {
                var state = response.getState();
                if (state === "SUCCESS") {
                    this.showToast('success', $A.get("$Label.c.SuccessfullyDeleted"));
                    component.set("v.listDeleteTargetId", []);
                    component.set("v.listSelectedData", []);
                    component.find("selectAll").set("v.checked", false);
        ...
        doAddShippingFee : function (component, event, helper, draftValues) {
            console.log('[doAddShippingFee] Start ==============================>');
            var objOrder = component.get("v.objOrder");
            var action = component.get("c.doAddShippingFee");

            console.log('[doAddShippingFee] data ==============================> ');
            ...
            action.setCallback(this, function (response) {
                ...
                if (state === "SUCCESS") {
                    ...
                    if(result['strStatus'] === 'SUCCESS') {
                        helper.showToast('success',  $A.get("$Label.c.SuccessfullyAddShipping"));
                        console.log('[doAddShippingFee] SUCCESS!!! ==============================>');
                        component.set("v.hasShippingFee", true);
                        component.set("v.isAvailableAddShipping", false);
                        $A.get('e.force:refreshView').fire();
                    } else {
                        this.showToast("error", result['strMessage']);
                        helper.getInitData(component, event, helper);
                    }
                } else if (state === "ERROR") {
                    console.log('[doAddShippingFee] ERROR!!! ==============================>');
            ...
            $A.enqueueAction(action);
            console.log('[doAddShippingFee] End ==============================>');
        },

        doSetDisability : function (component, event, helper) {
            console.log('[doSetDisability] Start ==============================>');
            var isPartnerProfile = component.get("v.isPartnerProfile");     // Partner Profile 일 경우 true
            var objOrder = component.get("v.objOrder");
            var listData = component.get("v.listData");
            var hasShippingFee = component.get("v.hasShippingFee");

            /* isAble... 의 값이 true일 때 수정 가능*/
            var isAbleEditQuantity = false;
            var isAbleEditUnitPrice = false;
            var isAbleEditDiscount = false;
            var isAbleEditRemark = false;
            var isAbleClickAddProduct = false;
            var isAbleClickSave = false;
            var isAbleClickDelete = false;
            /* isDisable... 의 값이 true일 때 수정 가능*/
            var isDisableButton = false;
            var isAbleClickAddShipping = false;
            var isAvailableAddShipping = false;

            if (isPartnerProfile == true) {
                if (objOrder.TRANS_METH__c == '02' || objOrder.TRANS_METH__c == '03') {
                    isAvailableAddShipping = true;
                }
                if (objOrder.Status == 'PO Registration') {
                    isAbleEditRemark = true;
                    isAbleClickSave = true;
                    isAbleClickDelete = true;
                    isAbleClickAddProduct = true;
                    if (hasShippingFee == false) {
                        isAbleEditQuantity = true;
                        isAbleClickAddShipping = true;
                    }
                } else if (objOrder.Status == 'Under Review' || objOrder.Status == 'Order Processing'){
                    isAbleEditRemark = true;
                    isAbleClickSave = true;
                } else {
                ...
                isAbleClickDelete = true;
                isAbleClickAddProduct = true;
                isAbleEditUnitPrice = true;
                isAbleEditDiscount = true;
                isAbleClickAddShipping = true;
                if (hasShippingFee == false) {
                    isAbleEditQuantity = true;
                }
                ...
                } else if (objOrder.Status == 'Under Review'){

                } else if (objOrder.Status == 'Order Processing') {

                } else {
                    isAbleEditQuantity = false;
                    isDisableButton = true;
                    isAbleEditDiscount = false;
                    isAbleEditRemark = false;
                    isAbleEditUnitPrice = false;
                }
            }

            component.set("v.isAbleEditQuantity", isAbleEditQuantity);
            component.set("v.isAbleEditUnitPrice", isAbleEditUnitPrice);
            component.set("v.isAbleEditDiscount", isAbleEditDiscount);
            component.set("v.isAbleEditRemark", isAbleEditRemark);
            component.set("v.isAbleClickAddProduct", isAbleClickAddProduct);
            component.set("v.isAbleClickSave", isAbleClickSave);
            component.set("v.isAbleClickDelete", isAbleClickDelete);
            component.set("v.isDisableButton", isDisableButton);
            component.set("v.isAbleClickAddShipping", isAbleClickAddShipping);
            component.set("v.isAvailableAddShipping", isAvailableAddShipping);
            console.log('[doSetDisability] End ==============================>');
        }
});